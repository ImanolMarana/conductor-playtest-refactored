...
public class AsyncSystemTaskExecutor {
    ...
    if (task == null) {
      handleTaskNotFound(systemTask, taskId);
      return;
    }
    ...
    if (handleTerminalStates(task, queueName)) {
      return;
    }

    if (handleRateLimiting(task, queueName)) {
      return;
    }

    executeSystemTask(systemTask, taskId, task, queueName);
  }

  private void handleTaskNotFound(WorkflowSystemTask systemTask, String taskId) {
  ...
  private boolean handleTerminalStates(TaskModel task, String queueName) {
  ...
  private boolean handleRateLimiting(TaskModel task, String queueName) {
    if (task.getStatus().equals(TaskModel.Status.SCHEDULED)) {
      if (executionDAOFacade.exceedsInProgressLimit(task)) {
        LOGGER.warn("Concurrent Execution limited for {}:{}", task.getTaskId(), task.getTaskDefName());
  ...
  private void executeSystemTask(
      WorkflowSystemTask systemTask, String taskId, TaskModel task, String queueName) {
    ...
    try {
      ...
      if (handleWorkflowTerminalState(workflow, task)) {
      ...
      executeBasedOnTaskStatus(systemTask, task, workflow);
      ...
      handleTaskQueueUpdate(systemTask, task, queueName);
  ...
  private boolean handleWorkflowTerminalState(WorkflowModel workflow, TaskModel task) {
  ...
  private void executeBasedOnTaskStatus(WorkflowSystemTask systemTask, TaskModel task, WorkflowModel workflow) {
  ...
  private void handleTaskQueueUpdate(WorkflowSystemTask systemTask, TaskModel task, String queueName) {
    boolean isTaskAsyncComplete = systemTask.isAsyncComplete(task);
