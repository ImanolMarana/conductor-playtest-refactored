...
public class Join extends WorkflowSystemTask {
    ...
    public boolean execute(
      ...
      List<String> joinOn = getJoinOnTaskRefNames(task);
      ...
      for (String joinOnRef : joinOn) {
        ...
        recordOptionalTaskFailure(optionalTaskFailures, forkedTask, taskStatus);
      }

      if (allDone || hasFailures || optionalTaskFailures.length() > 0) {
        return finalizeTaskStatus(task, hasFailures, failureReason, optionalTaskFailures);
      }
      return false;
    }

    private List<String> getJoinOnTaskRefNames(TaskModel task) {
    ...
    private boolean finalizeTaskStatus(
        TaskModel task,
        boolean hasFailures,
        StringBuilder failureReason,
        StringBuilder optionalTaskFailures) {
      if (hasFailures) {
    ...
    private void recordOptionalTaskFailure(
        StringBuilder optionalTaskFailures, TaskModel forkedTask, TaskModel.Status taskStatus) {
      if (forkedTask.getWorkflowTask().isOptional()
          && taskStatus == TaskModel.Status.COMPLETED_WITH_ERRORS) {
        optionalTaskFailures
            .append(
                String.format(
                    "%s/%s", forkedTask.getTaskDefName(), forkedTask.getTaskId()))
            .append(" ");
      }
    }
