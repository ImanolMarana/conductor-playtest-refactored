...
public class DeciderService {
    ...
    Optional<TaskModel> retry(
        ...
        if (shouldNotRetry(task, workflowTask, expectedRetryCount, retryCount)) {
            handleTaskFailure(task, workflow, workflowTask);
        }

        // retry... - but not immediately - put a delay...
        int startDelay = calculateRetryDelay(taskDefinition, task);

        return Optional.of(rescheduleTask(task, startDelay, taskDefinition, workflow, workflowTask));
    }

    private boolean shouldNotRetry(TaskModel task, WorkflowTask workflowTask, int expectedRetryCount, int retryCount) {
        return !task.getStatus().isRetriable()
                || TaskType.isBuiltIn(task.getTaskType())
                || expectedRetryCount <= retryCount
                || (workflowTask != null && workflowTask.isOptional());
    }

    private void handleTaskFailure(TaskModel task, WorkflowModel workflow, WorkflowTask workflowTask) {
        if (workflowTask != null && workflowTask.isOptional()) {
            return;
        }
        WorkflowModel.Status status = getWorkflowStatusForTaskFailure(task);
    ...
    private WorkflowModel.Status getWorkflowStatusForTaskFailure(TaskModel task) {
        switch (task.getStatus()) {
            case CANCELED:
                return WorkflowModel.Status.TERMINATED;
            case TIMED_OUT:
                return WorkflowModel.Status.TIMED_OUT;
            default:
                return WorkflowModel.Status.FAILED;
        }
    }

    private TaskModel rescheduleTask(TaskModel task, int startDelay, TaskDef taskDefinition, WorkflowModel workflow, WorkflowTask workflowTask) {
        task.setRetried(true);
    ...
    private int calculateRetryDelay(TaskDef taskDefinition, TaskModel task) {
