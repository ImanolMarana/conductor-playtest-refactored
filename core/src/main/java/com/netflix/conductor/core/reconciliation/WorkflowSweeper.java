...
public class WorkflowSweeper {
    ...
    void unack(WorkflowModel workflowModel, long workflowOffsetTimeout) {
      long postponeDurationSeconds = calculatePostponeDurationSeconds(workflowModel, workflowOffsetTimeout);
      queueDAO.setUnackTimeout(
          DECIDER_QUEUE, workflowModel.getWorkflowId(), postponeDurationSeconds * 1000);
    }
  
    private long calculatePostponeDurationSeconds(WorkflowModel workflowModel, long workflowOffsetTimeout) {
      for (TaskModel taskModel : workflowModel.getTasks()) {
        if (taskModel.getStatus() == Status.IN_PROGRESS) {
          return calculatePostponeDurationForInProgressTask(taskModel, workflowOffsetTimeout);
        } else if (taskModel.getStatus() == Status.SCHEDULED) {
          return calculatePostponeDurationForScheduledTask(workflowModel, taskModel);
        }
      }
      return 0;
    }
  
    private long calculatePostponeDurationForInProgressTask(TaskModel taskModel, long workflowOffsetTimeout) {
      if (taskModel.getTaskType().equals(TaskType.TASK_TYPE_WAIT)) {
        return calculatePostponeDurationForWaitTask(taskModel, workflowOffsetTimeout);
      } else if (taskModel.getTaskType().equals(TaskType.TASK_TYPE_HUMAN)) {
        return workflowOffsetTimeout;
      } else {
        return (taskModel.getResponseTimeoutSeconds() != 0)
            ? taskModel.getResponseTimeoutSeconds() + 1
            : workflowOffsetTimeout;
      }
    }
  
    private long calculatePostponeDurationForWaitTask(TaskModel taskModel, long workflowOffsetTimeout) {
      if (taskModel.getWaitTimeout() == 0) {
        return workflowOffsetTimeout;
      } else {
        long deltaInSeconds = (taskModel.getWaitTimeout() - System.currentTimeMillis()) / 1000;
        return (deltaInSeconds > 0) ? deltaInSeconds : 0;
      }
    }
  
    private long calculatePostponeDurationForScheduledTask(WorkflowModel workflowModel, TaskModel taskModel) {
      Optional<TaskDef> taskDefinition = taskModel.getTaskDefinition();
      if (taskDefinition.isPresent()) {
        return calculatePostponeDurationForScheduledTaskWithDefinition(workflowModel, taskDefinition.get());
      } else {
        return (workflowModel.getWorkflowDefinition().getTimeoutSeconds() != 0)
            ? workflowModel.getWorkflowDefinition().getTimeoutSeconds() + 1
            : workflowModel.getWorkflowDefinition().getTimeoutSeconds();
      }
    }
  
    private long calculatePostponeDurationForScheduledTaskWithDefinition(WorkflowModel workflowModel, TaskDef taskDef) {
      if (taskDef.getPollTimeoutSeconds() != null && taskDef.getPollTimeoutSeconds() != 0) {
        return taskDef.getPollTimeoutSeconds() + 1;
      } else {
        return (workflowModel.getWorkflowDefinition().getTimeoutSeconds() != 0)
            ? workflowModel.getWorkflowDefinition().getTimeoutSeconds() + 1
            : workflowModel.getWorkflowDefinition().getTimeoutSeconds();
