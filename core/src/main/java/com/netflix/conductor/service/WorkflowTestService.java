...
public class WorkflowTestService {
    ...
    do {
      ...
      if (loopCount > MAX_LOOPS) {
        return workflow; // Short circuit to avoid large loops
      }

      if (isInputMissingForRunningTasks(workflow, mockData)) {
        break;
      }

      processRunningTasks(workflow, mockData);
  ...
  private boolean isInputMissingForRunningTasks(Workflow workflow, Map<String, List<WorkflowTestRequest.TaskMock>> mockData) {
  ...
  private void processRunningTasks(Workflow workflow, Map<String, List<WorkflowTestRequest.TaskMock>> mockData) {
    workflow.getTasks().stream()
        .filter(t -> !t.getStatus().isTerminal())
        .forEach(running -> {
          if (running.getTaskType().equals(TaskType.SUB_WORKFLOW.name())) {
            handleSubWorkflow(request, running);
          }

          String refName = running.getReferenceTaskName();
          List<WorkflowTestRequest.TaskMock> taskMock = mockData.get(refName);
          if (taskMock == null || taskMock.isEmpty() || operators.contains(running.getTaskType())) {
            mockData.remove(refName);
            workflowService.decideWorkflow(workflowId);
          } else {
            processTaskMock(running, mockData, taskMock);
          }
        });
  }

  private void handleSubWorkflow(WorkflowTestRequest request, Task running) {
    String subWorkflowId = running.getSubWorkflowId();
    WorkflowTestRequest subWorkflowTestRequest = request.getSubWorkflowTestRequest().get(running.getReferenceTaskName());
    if (subWorkflowId != null && subWorkflowTestRequest != null) {
      testWorkflow(subWorkflowTestRequest, subWorkflowId);
    }
  }

  private void processTaskMock(Task running, Map<String, List<WorkflowTestRequest.TaskMock>> mockData, List<WorkflowTestRequest.TaskMock> taskMock) {
    WorkflowTestRequest.TaskMock task = taskMock.remove(0);
    if (task.getExecutionTime() > 0 || task.getQueueWaitTime() > 0) {
      updateTaskModel(running, task);
    } else {
      updateTaskResult(running, task);
    }
  }

  private void updateTaskModel(Task running, WorkflowTestRequest.TaskMock task) {
    TaskModel existing = executionDAO.getTask(running.getTaskId());
    existing.setScheduledTime(System.currentTimeMillis() - (task.getExecutionTime() + task.getQueueWaitTime()));
    existing.setStartTime(System.currentTimeMillis() - task.getExecutionTime());
    existing.setStatus(TaskModel.Status.valueOf(task.getStatus().name()));
    existing.getOutputData().putAll(task.getOutput());
    executionDAO.updateTask(existing);
    workflowService.decideWorkflow(workflowId);
  }

  private void updateTaskResult(Task running, WorkflowTestRequest.TaskMock task) {
