...
public class DefaultEventQueueProcessor {
    ...
    queue.observe()
        .subscribe(
            (Message msg) -> {
              try {
                processMessage(msg, queue, status);
  ...
  private void processMessage(Message msg, ObservableQueue queue, Status status) throws Exception {
    ...
    Optional<TaskModel> optionalTaskModel = findTask(json, workflow, payload);

    if (optionalTaskModel.isEmpty()) {
      LOGGER.error(
          "No matching tasks found to be marked as completed for workflow {}, taskRefName {}, taskId {}",
          workflowId,
          getValue("taskRefName", json),
          getValue("taskId", json));
      queue.ack(Collections.singletonList(msg));
      return;
    }

    updateTaskStatus(optionalTaskModel.get(), status, payloadJSON);
  ...
  private Optional<TaskModel> findTask(JsonNode json, WorkflowModel workflow, String payload) {
  ...
  private void updateTaskStatus(TaskModel taskModel, Status status, JsonNode payloadJSON) {
