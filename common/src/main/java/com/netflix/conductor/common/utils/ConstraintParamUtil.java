...
public class ConstraintParamUtil {
    ...
    private static List<String> extractParamPathComponentsFromString(
        ...
        if (value == null) {
            errorList.add(String.format("key: %s input parameter value: is null", key));
            return errorList;
        }
        ...
        for (String s : values) {
            if (s.startsWith("${") && s.endsWith("}")) {
                errorList.addAll(validateParamPath(s, key, taskName, workflow));
            }
        }
        return errorList;
    }

    private static List<String> validateParamPath(String s, String key, String taskName, WorkflowDef workflow) {
        List<String> errorList = new ArrayList<>();
        String paramPath = s.substring(2, s.length() - 1);

        if (StringUtils.containsWhitespace(paramPath)) {
            errorList.add(String.format("key: %s input parameter value: %s is not valid", key, paramPath));
        } else if (EnvUtils.isEnvironmentVariable(paramPath)) {
            validateEnvironmentVariable(paramPath, key, taskName, errorList);
        } else {
            validateWorkflowTaskReference(paramPath, key, taskName, workflow, errorList);
        }
        return errorList;
    }

    private static void validateEnvironmentVariable(String paramPath, String key, String taskName, List<String> errorList) {
        if (!isPredefinedEnum(paramPath)) {
            ...
            if (sysValue == null) {
                String errorMessage =
                        String.format(
                                ...
                                paramPath, taskName, key, "");
                errorList.add(errorMessage);
            }
        }
    }

    private static boolean isPredefinedEnum(String paramPath) {
    ...
    private static void validateWorkflowTaskReference(String paramPath, String key, String taskName, WorkflowDef workflow, List<String> errorList) {
