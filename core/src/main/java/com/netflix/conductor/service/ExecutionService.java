...
public class ExecutionService {
    ...
    for (String taskId : taskIds) {
      try {
        Optional<Task> task = pollTask(queueName, taskId, workerId);
  ...
  private Optional<Task> pollTask(String queueName, String taskId, String workerId) {
    ...
    taskModel.setCallbackAfterSeconds(0); // reset callbackAfterSeconds when giving the task to the
    // worker
    taskModel.setWorkerId(workerId);
    taskModel.incrementPollCount();
    executionDAOFacade.updateTask(taskModel);
    return Optional.of(taskModel.toTask());
  }
//Refactoring end
